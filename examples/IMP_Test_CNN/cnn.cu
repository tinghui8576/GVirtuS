...
int main() {
    const int batch_size = 10000; // 一次处理 1000 张
    ...
    unsigned int count = batch_size; // 只读 1000 张用于本次 batch
    ...
    double err_rate = (double)errors / (double)batch_size * 100.0;
    double acc = 100.0 - err_rate;
    printf("Batch size = %d\n", batch_size);
    printf("Error Rate = %.3f%% (%u/%d)\n", err_rate, errors, batch_size);
    printf("Accuracy   = %.3f%% (%d/%d)\n", acc, batch_size - errors, batch_size);
    printf("Elapsed    = %.3f ms (single batch forward)\n", ms);
