...
public:
	...
	// expose for host readback in main
    // Host/device staging
    float *d_input = nullptr;          // persistent device input (28x28)
	float* h_input_pinned = nullptr;    // host pinned (28x28) for graph H2D
	float *h_output_pinned = nullptr;
	bool graphbuilt = false;

	const float* host_output() const { return h_output_pinned; }
...

	{
		...
		// cudaHostAlloc(&h_output_pinned, 10 * sizeof(float), cudaHostAllocDefault);
        cudaHostAlloc(&h_input_pinned, 28 * 28 * sizeof(float), cudaHostAllocDefault);

	}

	void forward_pass (float data[28][28]) {
		std::memcpy(h_input_pinned, data, 28*28*sizeof(float));
		// cudaMemcpyAsync(d_input, data, 28*28*sizeof(float), cudaMemcpyHostToDevice, stream);
		// Replay graph
		

		// float (*input)[28];
		// cudaMalloc((void**)&input, 28 * 28 * sizeof(float)); // Allocate 2D on device
		// cudaMemcpy(input, h_input_pinned, 28 * 28 * sizeof(float), cudaMemcpyHostToDevice); // Copy host to device
		...
		if (!graphbuilt){
			...
			auto d_input_2d = reinterpret_cast<float (*)[28]>(d_input);
			

			// cudaMallocAsync(reinterpret_cast<void **>(&d_pre_output),   6*24*24*sizeof(float), stream);
			// cudaMallocAsync(reinterpret_cast<void **>(&d_output),       6*24*24*sizeof(float), stream);
			// cudaMallocAsync(reinterpret_cast<void **>(&d_pre_output_ss1), 6*6*6*sizeof(float), stream);
			// cudaMallocAsync(reinterpret_cast<void **>(&d_output_ss1),     6*6*6*sizeof(float), stream);
			// cudaMallocAsync(reinterpret_cast<void **>(&d_pre_output_fc), 10*sizeof(float), stream);
			// cudaMallocAsync(reinterpret_cast<void **>(&d_output_fc),     10*sizeof(float), stream);
			
			

			cudaMemcpyAsync(d_input, h_input_pinned, 28 * 28 * sizeof(float), cudaMemcpyHostToDevice, stream);
			cudaMallocAsync((void**)(&d_pre_output),   6*24*24*sizeof(float), stream);
			// Assuming input, pre_output, and weight are already allocated and initialized in device memory
			kernel_conv_filter << <numBlocks, threadsPerBlock, 0, stream >> > (d_input_2d, d_pre_output, d_weights);
			/*BIAS*/
			kernel_conv_bias << <numBlocks, threadsPerBlock, 0, stream >> > (d_pre_output, d_bias);
			/*SIGMOID*/
			cudaMallocAsync((void**)(&d_output),       6*24*24*sizeof(float), stream);
			kernel_conv_sigmoid << <numBlocks, threadsPerBlock, 0, stream >> > (d_pre_output, d_output);
			cudaFreeAsync(d_pre_output,    stream);
			...
			cudaMallocAsync((void**)(&d_pre_output_ss1), 6*6*6*sizeof(float), stream);
			kernel_ss1_filter << <numBlocksSS, threadsPerBlockSS, 0, stream >> > (d_output, d_pre_output_ss1, d_weights_ss1);
			cudaFreeAsync(d_output,        stream);
			/*SS BIAS*/
			// Assuming pre_output and bias are already allocated and populated on the device
			kernel_ss1_bias << <numBlocksSS, threadsPerBlockSS, 0, stream >> > (d_pre_output_ss1, d_bias_ss1);
			/*SS SIGMOID*/
			cudaMallocAsync((void**)(&d_output_ss1),     6*6*6*sizeof(float), stream);
			kernel_ss2_sigmoid << <numBlocksSS, threadsPerBlockSS, 0, stream >> > (d_pre_output_ss1, d_output_ss1);
			cudaFreeAsync(d_pre_output_ss1,stream);
			/*FULLY CONNECTED*/
			/*FC MAPPING to dim 10*/
			/*dim3 threadsPerBlockFC(10);
			dim3 numBlocksFC(1);*/
			cudaMallocAsync((void**)(&d_pre_output_fc), 10*sizeof(float), stream);
			kernel_fc1 << <numBlocks, threadsPerBlock, 0, stream >> > (d_output_ss1, d_pre_output_fc, d_weights_fc);
			cudaFreeAsync(d_output_ss1,    stream);
			/*FC BIAS*/
			kernel_fc1_bias << <1, 10, 0, stream >> > (d_pre_output_fc, d_bias_fc);
			/*FC SIGMOID*/
			cudaMallocAsync((void**)(&d_output_fc),     10*sizeof(float), stream);

			kernel_fc1_sigmoid << <1, 10, 0, stream >> > (d_pre_output_fc, d_output_fc);
			cudaFreeAsync(d_pre_output_fc, stream);
			cudaMemcpyAsync(h_output_pinned, d_output_fc, 10*sizeof(float), cudaMemcpyDeviceToHost, stream);

			cudaFreeAsync(d_output_fc,     stream);
			
			

			// cudaFreeAsync(h_output_pinned, stream);
	...
	~Layer() {
		...
		cudaFree(d_weights_fc);     // missing
		cudaFree(d_pre_output_fc);  // missing
		cudaFree(d_bias_fc);
		cudaFree(d_output_fc);      // missing
		cudaFree(d_input);          // missing
...
int main()
{
	...
	int counts[] = {1, 10, 100, 1000, 10000};
	...
	std::ofstream csvFile("Graph.csv");
    csvFile << "NSample,ExecutionTime(ms),AvgTime(ms)\n";

	for (int numsample : counts) {
		//Creating layer
		Layer layer(6, 5, 5);
		...
		for (unsigned int i = 0; i < numsample; i++) {
			...
			std::memcpy(res, layer.host_output(), 10 * sizeof(float));
			for (int j = 0; j < 10; j++) {
				if (res[max] < res[j])
					max = j;
			}
		...
		// unsigned int test_cnt = numsample;
		...
		printf("%d samples for Ex time = %f (ms) and Avg time = %f (ms) and Accuracy = %.3f%%\n", numsample, time_taken, time_taken/double(numsample), 100.0 - double(error) / double(numsample) * 100.0);
		csvFile << numsample << "," << time_taken << "," << time_taken/double(numsample) << "\n";
	}
    return 0;
}