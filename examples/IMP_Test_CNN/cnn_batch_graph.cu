...
public:
    ...
    // pinned host buffers (stable addresses captured in the graph)
    ...
    void forward_pass_batch(float (*data)[28][28], int batch_size) {
        ...
        if (!graph_built){
            ...
            // CUDA_CHECK(cudaStreamSynchronize(stream)); // ensure ready
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < total_count; ++i) data_set[i] = (mnist_data*)malloc(sizeof(mnist_data));

    unsigned int loaded = (unsigned int)total_count;
    if (mnist_load(images_path, labels_path, data_set, &loaded) != 0) {
        fprintf(stderr, "MNIST load failed\n");
        return 1;
    }
    total_count = (int)loaded;
    ...
    
    {
        ...
        float (*batch_data)[28][28] = (float(*)[28][28])malloc((size_t)batch_size * INSIZE * INSIZE  * sizeof(float));
