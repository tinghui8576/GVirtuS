...
public:
    ...
    int captured_batch = -1;
    size_t in_bytes = 0, conv_bytes = 0, ss_bytes = 0, fc_bytes = 0;
    ...
    ~Layer() {
        ...
        // if (d_conv_pre)  cudaFree(d_conv_pre);
        // if (d_conv_out)  cudaFree(d_conv_out);
        // if (d_ss_pre)    cudaFree(d_ss_pre);
        // if (d_ss_out)    cudaFree(d_ss_out);
        // if (d_fc_pre)    cudaFree(d_fc_pre);
        // if (d_out_fc) cudaFree(d_out_fc);

        if (d_in)        cudaFreeAsync(d_in,     stream);
        if (d_conv_pre)  cudaFreeAsync(d_conv_pre,     stream);
        if (d_conv_out)  cudaFreeAsync(d_conv_out,     stream);
        if (d_ss_pre)    cudaFreeAsync(d_ss_pre,     stream);
        if (d_ss_out)    cudaFreeAsync(d_ss_out,     stream);
        if (d_fc_pre)    cudaFreeAsync(d_fc_pre,     stream);
        if (d_out_fc)    cudaFreeAsync(d_out_fc,     stream);
    ...
    void forward_pass_batch(float (*data)[28][28], int batch_size) {
        ...
        in_bytes   = (size_t)batch_size * INSIZE * INSIZE * sizeof(float);
        conv_bytes = (size_t)batch_size * CONV_C * CONV_OUT * CONV_OUT * sizeof(float);
        ss_bytes   = (size_t)batch_size * CONV_C * SS_OUT * SS_OUT * sizeof(float);
        fc_bytes   = (size_t)batch_size * FC_OUT * sizeof(float);
        ...
        if (!graph_built){
            ...
            CUDA_CHECK(cudaMallocAsync((void**)&d_in,       in_bytes,   stream));
            CUDA_CHECK(cudaMallocAsync((void**)&d_conv_pre, conv_bytes, stream));
            CUDA_CHECK(cudaMallocAsync((void**)&d_conv_out, conv_bytes, stream));
            CUDA_CHECK(cudaMallocAsync((void**)&d_ss_pre,   ss_bytes,   stream));
            CUDA_CHECK(cudaMallocAsync((void**)&d_ss_out,   ss_bytes,   stream));
            CUDA_CHECK(cudaMallocAsync((void**)&d_fc_pre,   fc_bytes,   stream));
            CUDA_CHECK(cudaMallocAsync((void**)&d_out_fc,   fc_bytes,   stream));
            // logMemoryStatus("after cudaMallocAsync");
            graph_built = true;
        }

        captured_batch = batch_size;
        ...
        if (batch_size != captured_batch) {
            fprintf(stderr, "Error: actual_batch (%d) != captured_batch (%d)\n",
        ...
        dim3 bC((CONV_OUT + tC.x - 1) / tC.x,
                ...
                batch_size * CONV_C);
        // CUDA_CHECK(cudaMalloc(&d_conv_pre, conv_bytes));
        kernel_conv_filter<<<bC, tC, 0, stream >>>(d_in, d_conv_pre, d_wconv, batch_size); 
        // cudaFreeAsync(d_in,     stream);
        CUDA_CHECK(cudaGetLastError());
        kernel_conv_bias<<<bC, tC, 0, stream >>>(d_conv_pre, d_bconv, batch_size);
        CUDA_CHECK(cudaGetLastError());
        // CUDA_CHECK(cudaMalloc(&d_conv_out, conv_bytes));
        kernel_conv_sigmoid<<<bC, tC, 0, stream >>>(d_conv_pre, d_conv_out, batch_size);
        CUDA_CHECK(cudaGetLastError());
        // cudaFreeAsync(d_conv_pre,     stream);

        // ===== Subsampling =====
        dim3 tS(6, 6);
        dim3 bS(1, 1, batch_size * CONV_C);
        // CUDA_CHECK(cudaMalloc(&d_ss_pre,   ss_bytes));
        kernel_ss1_filter<<<bS, tS, 0, stream >>>(d_conv_out, d_ss_pre, d_wss, batch_size);
        // cudaFreeAsync(d_conv_out,     stream);
        CUDA_CHECK(cudaGetLastError());
        kernel_ss1_bias<<<bS, tS, 0, stream >>>(d_ss_pre, d_bss, batch_size);
        CUDA_CHECK(cudaGetLastError());
        // CUDA_CHECK(cudaMalloc(&d_ss_out,   ss_bytes));
        kernel_ss_sigmoid<<<bS, tS, 0, stream >>>(d_ss_pre, d_ss_out, batch_size);
        CUDA_CHECK(cudaGetLastError());
        // cudaFreeAsync(d_ss_pre,     stream);       
        ...
        // CUDA_CHECK(cudaMalloc(&d_fc_pre,   fc_bytes));
        kernel_fc1<<<bF, tF, 0, stream >>>(d_ss_out, d_fc_pre, d_wfc, batch_size);
        // cudaFreeAsync(d_ss_out,     stream);
        CUDA_CHECK(cudaGetLastError());
        kernel_fc1_bias<<<bF, tF, 0, stream >>>(d_fc_pre, d_bfc, batch_size);
        CUDA_CHECK(cudaGetLastError());
        // CUDA_CHECK(cudaMalloc(&d_out_fc,   fc_bytes));
        kernel_fc1_sigmoid<<<bF, tF, 0, stream >>>(d_fc_pre, d_out_fc, batch_size);
        CUDA_CHECK(cudaGetLastError());
        // cudaFreeAsync(d_fc_pre,     stream);
        cudaMemcpyAsync(h_output_pinned, d_out_fc, fc_bytes, cudaMemcpyDeviceToHost, stream);
        // cudaFreeAsync(d_out_fc,     stream);
...
static int parse_batch_size(int argc, char** argv, int max_count) {
    int v = 1000;
    if (argc > 1) {
        try { v = std::stoi(std::string(argv[1])); } catch (...) {}
    }
    if (v < 1) v = 1;
...
int main(int argc, char** argv) {
    ...
    if (mnist_load(images_path, labels_path, data_set, (unsigned int*)&total_count) != 0) {
    ...
    // int batch_sizes[] = {4000};
    // int num_batch_sizes = sizeof(batch_sizes) / sizeof(int);
    const int N_runs = 1000;
    
    std::string fname = "Batch.csv";
    std::ifstream fin(fname, std::ios::binary | std::ios::ate);
    bool need_header = !fin.is_open() || fin.tellg() == 0;
    fin.close();

    std::ofstream csvFile(fname, std::ios::app);
    if (need_header) {
    ...
    
    {
        ...
        float (*batch_data)[28][28] = (float(*)[28][28])malloc((size_t)batch_size * 28 * 28 * sizeof(float));
        ...
        for (int i=0; i < N_runs; i++) {
            ...
            for (int i = 0; i < batch_size; ++i) {
                int argmax = 0;
                float best = h_out[i * FC_OUT];
                for (int j = 1; j < FC_OUT; ++j) {
                    float v = h_out[i * FC_OUT + j];
        ...
        // final totals
        // const double total_err_rate = (double)total_errors / (double)count * 100.0;
        // const double total_acc = 100.0 - total_err_rate;
        // printf("batch=%u  acc=%.3f%%  time=%.3f ms avgtime=%.3f ms\n",
        //     batch_size, total_acc, total_ms, total_ms/((double)count/(double)batch_size));
        // csvFile << batch_size << "," << total_ms << "," << total_ms/((double)count/(double)batch_size) << "\n";
