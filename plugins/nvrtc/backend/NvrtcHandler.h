...
private:
    ...
    void Initialize();
    typedef std::shared_ptr<gvirtus::communicators::Result> (*NvrtcRoutineHandler)(NvrtcHandler *,
        std::shared_ptr<gvirtus::communicators::Buffer>);
    static std::map<std::string, NvrtcRoutineHandler> * mspHandlers;
};

#define NVRTC_ROUTINE_HANDLER(name) std::shared_ptr<gvirtus::communicators::Result> handle##name(NvrtcHandler * pThis, std::shared_ptr<gvirtus::communicators::Buffer> in)
