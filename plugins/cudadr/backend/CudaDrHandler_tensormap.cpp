...
CUDA_DRIVER_HANDLER(TensorMapEncodeTiled) {
    ...
    const cuuint64_t* globalStrides = input_buffer->Assign<cuuint64_t>(tensorRank * sizeof(cuuint64_t));
    const cuuint32_t* boxDim = input_buffer->Assign<cuuint32_t>(tensorRank * sizeof(cuuint32_t));
    const cuuint32_t* elementStrides = input_buffer->Assign<cuuint32_t>(tensorRank * sizeof(cuuint32_t));
    ...
    CUresult exit_code = cuTensorMapEncodeTiled(&tensorMap, tensorDataType, tensorRank, globalAddress, globalDim, globalStrides, boxDim, elementStrides, interleave, swizzle, l2Promotion, oobFill);
