...
CUDA_DRIVER_HANDLER(MemGetAllocationGranularity) {
    ...
    LOG4CPLUS_DEBUG(pThis->GetLogger(), "cuMemGetAllocationGranularity executed, granularity: " << granularity);
...
CUDA_DRIVER_HANDLER(MemSetAccess) {
    ...
    LOG4CPLUS_DEBUG(pThis->GetLogger(), "cuMemSetAccess executed for ptr: " << ptr << ", size: " << size << ", count: " << count);
    return std::make_shared<Result>((cudaError_t) exit_code);
}
...
CUDA_DRIVER_HANDLER(MemUnmap) {
    ...
    LOG4CPLUS_DEBUG(pThis->GetLogger(), "cuMemUnmap executed for ptr: " << ptr << ", size: " << size);
    return std::make_shared<Result>((cudaError_t) exit_code);
}