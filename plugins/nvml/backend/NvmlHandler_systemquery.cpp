...
NVML_ROUTINE_HANDLER(SystemGetCudaDriverVersion_v2) {
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("SystemGetCudaDriverVersion_v2"));
    ...
    LOG4CPLUS_DEBUG(logger, "nvmlSystemGetCudaDriverVersion_v2 executed");
    return std::make_shared<Result>(result, out);
}

NVML_ROUTINE_HANDLER(SystemGetDriverVersion) {
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("SystemGetDriverVersion"));
    ...
    LOG4CPLUS_DEBUG(logger, "nvmlSystemGetDriverVersion executed");
    return std::make_shared<Result>(result, out);
}