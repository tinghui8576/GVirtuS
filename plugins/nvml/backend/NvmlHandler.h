...
private:
    ...
    void Initialize();
    typedef std::shared_ptr<gvirtus::communicators::Result> (*NvmlRoutineHandler)(NvmlHandler *,
        std::shared_ptr<gvirtus::communicators::Buffer>);
    static std::map<std::string, NvmlRoutineHandler> * mspHandlers;
};

#define NVML_ROUTINE_HANDLER(name) std::shared_ptr<gvirtus::communicators::Result> handle##name(NvmlHandler * pThis, std::shared_ptr<gvirtus::communicators::Buffer> in)
