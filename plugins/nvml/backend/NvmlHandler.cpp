...
extern "C" std::shared_ptr<NvmlHandler> create_t() {
    return std::make_shared<NvmlHandler>();
}
...
void NvmlHandler::setLogLevel(Logger *logger) {
...
std::shared_ptr<Result> NvmlHandler::Execute(std::string routine, std::shared_ptr<Buffer> input_buffer) {
