...
extern "C" nvmlReturn_t nvmlDeviceGetCount(unsigned int* deviceCount) {
    ...
    if (NvmlFrontend::Success())
        *deviceCount = NvmlFrontend::GetOutputVariable<unsigned int>();
...
extern "C" nvmlReturn_t nvmlDeviceGetTemperature(nvmlDevice_t device, nvmlTemperatureSensors_t sensorType, unsigned int *temp) {
    ...
    if (NvmlFrontend::Success())
        *temp = NvmlFrontend::GetOutputVariable<unsigned int>();
    return NvmlFrontend::GetExitCode();
}
...
extern "C" nvmlReturn_t nvmlDeviceGetEccMode(nvmlDevice_t device, nvmlEnableState_t *current, nvmlEnableState_t *pending) {
    ...
    if (NvmlFrontend::Success()) {
        nvmlEnableState_t *current_backend = NvmlFrontend::GetOutputHostPointer<nvmlEnableState_t>();
        std::memcpy(current, current_backend, sizeof(nvmlEnableState_t));
        nvmlEnableState_t *pending_backend = NvmlFrontend::GetOutputHostPointer<nvmlEnableState_t>();
...
extern "C" nvmlReturn_t nvmlDeviceGetFanSpeed(nvmlDevice_t device, unsigned int *speed) {
    ...
    if (NvmlFrontend::Success())
        *speed = NvmlFrontend::GetOutputVariable<unsigned int>();
    return NvmlFrontend::GetExitCode();
}
...
extern "C" nvmlReturn_t nvmlDeviceGetPerformanceState(nvmlDevice_t device, nvmlPstates_t *pState) {
    ...
    if (NvmlFrontend::Success())
        *pState = NvmlFrontend::GetOutputVariable<nvmlPstates_t>();
    return NvmlFrontend::GetExitCode();
}
...
extern "C" nvmlReturn_t nvmlDeviceGetPowerUsage(nvmlDevice_t device, unsigned int *power) {
    ...
    if (NvmlFrontend::Success())
        *power = NvmlFrontend::GetOutputVariable<unsigned int>();
    return NvmlFrontend::GetExitCode();
}
...
extern "C" nvmlReturn_t nvmlDeviceGetEnforcedPowerLimit(nvmlDevice_t device, unsigned int *limit) {
    ...
    if (NvmlFrontend::Success())
        *limit = NvmlFrontend::GetOutputVariable<unsigned int>();
    return NvmlFrontend::GetExitCode();
}
...
extern "C" nvmlReturn_t nvmlDeviceGetUtilizationRates(nvmlDevice_t device, nvmlUtilization_t *utilization) {
    ...
    if (NvmlFrontend::Success()) {
        nvmlUtilization_t *utilization_backend = NvmlFrontend::GetOutputHostPointer<nvmlUtilization_t>();
...
extern "C" nvmlReturn_t nvmlDeviceGetMigMode(nvmlDevice_t device, unsigned int *current, unsigned int *pending) {
    NvmlFrontend::Prepare();
    NvmlFrontend::AddHostPointerForArguments(&device);
...
extern "C" nvmlReturn_t nvmlDeviceGetMaxMigDeviceCount(nvmlDevice_t device, unsigned int *count) {
    ...
    if (NvmlFrontend::Success())
        *count = NvmlFrontend::GetOutputVariable<unsigned int>();
    return NvmlFrontend::GetExitCode();
}
...
extern "C" nvmlReturn_t nvmlDeviceIsMigDeviceHandle(nvmlDevice_t device, unsigned int *isMigDevice) {
    ...
    if (NvmlFrontend::Success())
        *isMigDevice = NvmlFrontend::GetOutputVariable<unsigned int>();
    return NvmlFrontend::GetExitCode();
}
