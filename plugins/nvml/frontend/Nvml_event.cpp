...
extern "C" nvmlReturn_t nvmlEventSetCreate(nvmlEventSet_t* set) {
    ...
    NvmlFrontend::AddDevicePointerForArguments(set);
    NvmlFrontend::Execute("nvmlEventSetCreate");
    if (NvmlFrontend::Success()) {
        *set = NvmlFrontend::GetOutputVariable<nvmlEventSet_t>();
    }
...
extern "C" nvmlReturn_t nvmlEventSetFree(nvmlEventSet_t set) {
    NvmlFrontend::Prepare();
    NvmlFrontend::AddDevicePointerForArguments(set);
    NvmlFrontend::Execute("nvmlEventSetFree");
    return NvmlFrontend::GetExitCode();
}