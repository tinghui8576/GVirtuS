...
VMSocketCommunicator::VMSocketCommunicator(const std::string &communicator) {
    ...
    if (pathptr == NULL) throw "VMSocketCommunicator: path not specified.";
    mDevice = string(deviceptr).substr(0, pathptr - deviceptr);
    mPath = string(pathptr + 1);
}

void VMSocketCommunicator::Serve() { throw "VMSocketCommunicator: cannot Serve"; }
...
void VMSocketCommunicator::Connect() {
    ...
    if (ioctl(mFd, 0, mPath.c_str()) != 0) throw "VMSocketCommunicator: cannot connect.";
}
...
void VMSocketCommunicator::Sync() { fsync(mFd); }

void VMSocketCommunicator::Close() { close(mFd); }
}  // namespace gvirtus