...
ZmqCommunicator::ZmqCommunicator(const std::string &communicator) {
    ...
    if (!initialized) {
        WSADATA data;
        if (WSAStartup(MAKEWORD(2, 2), &data) != 0) throw "Cannot initialized WinSock.";
        initialized = true;
    }
#endif
    ...
    free(hostname);
    if (ent == NULL) throw "ZmqCommunicator: Can't resolve hostname '" + mHostname + "'.";
...
void ZmqCommunicator::Serve() {
    ...
    if (result != 0) throw "ZmqCommunicator: Can't bind socket.";

    if (listen(mSocketFd, 5) != 0) throw "AfUnixCommunicator: Can't listen from socket.";
}
...
void ZmqCommunicator::Close() { throw "ZmqCommunicator::Close()\n"; }
...
void ZmqCommunicator::Sync() { mpOutput->flush(); }
