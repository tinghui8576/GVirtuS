...
using TransportSelector =
    std::function<std::optional<Transport>(const std::string &routine, size_t bytes_hint)>;
 ...
 *
 * Usage pattern:
 *   - After writing the routine name on TCP, call begin_call(routine, Transport::RDMA/TCP,
 * bytes_hint)
...
class HybridCommunicator : public Communicator {
   ...
   public:
    ...
    HybridCommunicator(std::shared_ptr<Communicator> tcp, std::shared_ptr<Communicator> rdma,
                       Transport default_transport = Transport::TCP)
        : _tcp(std::move(tcp)), _rdma(std::move(rdma)), _default_transport(default_transport) {}
