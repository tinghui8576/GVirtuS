...
std::shared_ptr<Communicator> HybridCommunicator::current_channel_locked() {
    ...
    switch (_current_call_transport.value()) {
        case Transport::TCP:
            if (!_tcp)
                throw std::runtime_error("HybridCommunicator: TCP channel is not available.");
            return _tcp;
        case Transport::RDMA:
            if (!_rdma)
                throw std::runtime_error("HybridCommunicator: RDMA channel is not available.");
            return _rdma;
        default:
...
// ----------------------
void HybridCommunicator::begin_call(const std::string& routine, Transport t, size_t bytes_hint) {
...
#ifdef DEBUG
    std::cout << "[Hybrid] end_call: routine=" << _current_call_routine << std::endl;
#endif
...
const Communicator* const HybridCommunicator::Accept() const {
    ...
    if (!tcp_raw || !rdma_raw) {
        ...
        std::cerr << "[Hybrid] Accept(): missing one of the channels (tcp="
                  << (tcp_raw ? "ok" : "null") << ", rdma=" << (rdma_raw ? "ok" : "null") << ")"
                  << std::endl;
#endif
        return nullptr;
    }
...
size_t HybridCommunicator::Read(char* buffer, size_t size) {
    ...
    //         read that header from TCP first.
    {
        ...
        if (_current_call_transport.has_value() &&
            _current_call_transport.value() == Transport::RDMA && _awaiting_header_rx &&
            _header_remaining_rx > 0) {
...
#ifdef DEBUG
    std::cout << "[Hybrid][Read] body(" << (ch.get() == _rdma.get() ? "RDMA" : "TCP")
...
size_t HybridCommunicator::Write(const char* buffer, size_t size) {
    ...
    //         write that header via TCP first.
    {
        ...
        if (_current_call_transport.has_value() &&
            _current_call_transport.value() == Transport::RDMA && _awaiting_header_tx &&
            _header_remaining_tx > 0) {
...
#ifdef DEBUG
    std::cout << "[Hybrid][Write] body(" << (ch.get() == _rdma.get() ? "RDMA" : "TCP")
...
void HybridCommunicator::Sync() {
    ...
    if (_current_call_transport.has_value() && _current_call_transport.value() == Transport::RDMA) {
...
extern "C" std::shared_ptr<gvirtus::communicators::HybridCommunicator> create_communicator(
    ...
    if (!hybrid_ep) {
        throw std::runtime_error(
            "HybridCommunicator factory: invalid endpoint type (expected Endpoint_Hybrid).");
    }
