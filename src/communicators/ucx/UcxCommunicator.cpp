...
void UcxCommunicator::_resolve_sockaddr(struct sockaddr_storage& ss, socklen_t& slen) const {
  ...
  if (rc != 0 || !res) {
    ...
    oss << "UcxCommunicator: getaddrinfo failed for " << _hostname << ":" << _port << " (" << gai_strerror(rc) << ")";
...
void UcxCommunicator::_send_loop() {
  while (!_closing && _send_thread_running) {
    ...
    try {
      ...
      std::cerr << "UcxCommunicator::_send_loop error: " << e.what() << std::endl;
      _closing = true;
      break;
    }
  }
  ...
  while (!_closing && _recv_thread_running) {
    try {
      ...
      if (rh.magic != kMagic || rh.version != kProtoVersion) {
          throw std::runtime_error("Protocol mismatch in received response header.");
      }
      ...
      std::shared_ptr<Waiter> w;
      {
        ...
        if (it != _waiters.end()) {
          w = it->second;
            ...
            std::cerr << "Warning: Received response for unknown or timed-out msg_id: " << rh.msg_id << std::endl;
            continue;
        }
      }
      ...
      std::cerr << "UcxCommunicator::_recv_loop error, closing connection: " << e.what() << std::endl;
      _closing = true;
...
UcxCommunicator::SubmitRequest(const char* routine_cstr,
  ...
  if (!_ep) throw std::runtime_error("SubmitRequest: endpoint not connected.");
...
std::string UcxCommunicator::to_string() {
...
void UcxCommunicator::_on_ep_error(void* arg, ucp_ep_h /*ep*/, ucs_status_t status) {
  auto* self = reinterpret_cast<UcxCommunicator*>(arg);
  std::cerr << "UCX endpoint error: " << _ucs_str(status) << ". Closing communicator." << std::endl;
  self->Close();
}
