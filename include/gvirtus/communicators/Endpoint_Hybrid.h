...
class Endpoint_Hybrid : public Endpoint {
   ...
   public:
    ...
    Endpoint_Hybrid(const std::string &suite, const std::string &protocol,
                    const std::string &address, const std::string &rdma_suite,
                    const std::string &rdma_port, const std::string &tcp_suite,
    ...
    virtual inline const std::string to_string() const override {
        return _suite + ":" + _protocol + "://" + _address + " (rdma=" + _rdma_suite + ":" +
               std::to_string(_rdma_port) + ", tcp=" + _tcp_suite + ":" +
               std::to_string(_tcp_port) + ")";
    }
};
 ...
 */
void from_json(const nlohmann::json &j, Endpoint_Hybrid &end);
}  // namespace gvirtus::communicators
